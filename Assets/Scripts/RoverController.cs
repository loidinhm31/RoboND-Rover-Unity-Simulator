...
public class RoverController : IRobotController
{
    ...
    {
        get { return new Vector3(rb.linearVelocity.x, 0, rb.linearVelocity.z); }
    }

    Vector3 VerticalVelocity
    {
        get { return new Vector3(0, rb.linearVelocity.y, 0); }
    }

    public override float Zoom
    {
        get { return cameraDefaultFOV / camera.fieldOfView; }
    }
...
//	bool isPickingUp;
    ...
    bool isFixedTurning;
    Action<PickupSample> pickupCallback;
...
//				wheelAngle += hRotateSpeed * Time.deltaTime;
        ...
        if (isMotorInput)
        {
            ...
            wheels[0].motorTorque = wheels[1].motorTorque =
                wheels[2].motorTorque = wheels[3].motorTorque = lastMoveInput * motorTorque;
            wheels[0].brakeTorque = wheels[1].brakeTorque =
                wheels[2].brakeTorque = wheels[3].brakeTorque = lastBrakeInput * brakeTorque;
...
//			wheels [ 2 ].steerAngle = wheels [ 3 ].steerAngle = -lastAngle * 2; // -45 * Mathf.Sign ( lastAngle );
        ...
        {
            speedPercent =
                GroundVelocity.magnitude / moveSpeed; // use the forward speed here so reverse can turn better
...
//		camera.fieldOfView = cameraDefaultFOV;
    ...
    public override void PickupObjective(Action<PickupSample> onPickup)
    {
        if (IsPickingUpSample)
            return;
...
//		yield return new WaitForSeconds ( 2 );
        ...
        Ray ray = new Ray(armActuator.foldedPosition.position,
            curObjective.GetCenterPosition() - armActuator.foldedPosition.position);
...
//			GetSample ( true );
    ...
    Vector3 GetPositionOnFlatbed()
    {
        ...
        throw new NullReferenceException("No more positions left on the rover!");
    }
}